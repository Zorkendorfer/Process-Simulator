"""Isentropic compressor with efficiency."""
...
class CompressorOp(IUnitOp):
    def __init__(self, fc: FlashCalculator, P_out: float, eta: float = 0.72):
        if not (0.0 < eta <= 1.0):
            raise ValueError("CompressorOp: eta must be in (0, 1]")
        self._fc = fc
        self._P_out = P_out
        self._eta = eta
        self.inlet = Stream()
    ...
    def set_inlet(self, port: str, stream: Stream) -> None:
        if port != "in":
            raise ValueError(f"CompressorOp: unknown inlet port '{port}'")
        self.inlet = stream

    def get_outlet(self, port: str) -> Stream:
        if port != "out":
            raise ValueError(f"CompressorOp: unknown outlet port '{port}'")
        return self.outlet

    def solve(self) -> None:
        if self.inlet.total_flow <= 0.0:
            raise ValueError("CompressorOp: inlet.total_flow must be > 0")
        if self._P_out <= self.inlet.P:
            raise ValueError("CompressorOp: P_out must be greater than inlet pressure")

        T_in = self.inlet.T; P_in = self.inlet.P; z = self.inlet.z
        H_in = self._fc.phase_enthalpy(T_in, P_in, z, False)
        S_in = self._fc.phase_entropy(T_in, P_in, z, False)

        r_s = self._fc.flash_PS(self._P_out, S_in, z, T_in)
        H_out_s = self._fc.total_enthalpy(r_s)

        W_act = (H_out_s - H_in) / self._eta
        H_out = H_in + W_act
